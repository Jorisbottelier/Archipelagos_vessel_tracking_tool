...
* Devides data into monthly chunks to prevent API timeouts.
* Downloads Global Fishing Watch data across a custom-defined maritime region spanning a large part of the eastern Mediterranean.
...
### 3. Apparent Fishing Effort Analysis (`AFE_map.py`, `AFE_bulk_map.py`, `AFE_report.py`)
* **Grid-Based Mapping:** Aggregates fishing hours onto a spatial vector grid.
* **Interactive Heatmaps:** Renders high-fidelity Folium heatmaps layered under an interactive matrix that shows fishing activity per country and per vessel.
* **National Border Analysis:** Automatically groups and summarizes fishing activitiy in territorial waters of Greece (GRC), Turkey (TUR), Italy (ITA), and Malta (MLT), and the Fisheries Management Zone (FMZ) of Malta. 
